...
mapParseResult f (Result i a) = Result i (f a)
...
satisfy test =
  ...
  if test c then valueParser c else unexpectedCharParser c
  )
...
natural ::
  Parser Int
natural =
  list digit `flbindParser` (\k -> case read k of
    Empty -> failed
...
surnameParser ::
  Parser Chars
surnameParser = (\u v w -> u :. v ++ w) <$> upper <*> thisMany 5 lower <*> list lower
  -- do
  -- firstC <- upper
  -- next5 <- thisMany 5 lower
  -- rest <- list lower
  -- return (firstC :. next5 ++ rest)
...
personParser =
  ...
  -- _age <- ageParser
  -- spaces1
  -- _firstName <- firstNameParser
  -- spaces1
  -- _surname <- surnameParser
  -- spaces1
  -- _smoker <- smokerParser
  -- spaces1
  -- _phone <- phoneParser
  -- return (Person _age _firstName _surname _smoker _phone)
